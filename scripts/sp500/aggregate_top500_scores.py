...
def _score_with_model(df: pd.DataFrame, model_path: Path) -> Tuple[pd.DataFrame, List[str]]:
    ...
    try:
        best_iter_int = int(best_iter) if best_iter is not None else None
    except Exception:
        best_iter_int = None
    out["score"] = booster.predict(X, num_iteration=(best_iter_int if best_iter_int and best_iter_int > 0 else None))
    return out, feats
...
def aggregate_topN(
    ...
    if generate_ndcg:
        try:
            from scripts.backtest.backtest import backtest_from_aggregated_scores
